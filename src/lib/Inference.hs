...
inferModule scope (UModule uDecl sourceMap) = do
  (evaluated, ((bindings, synthCandidates), decls)) <- runUInferM mempty scope mempty do
    subst <- inferUDecl uDecl
    extendR subst $ substBuilderR $ EvaluatedModule mempty mempty sourceMap
...
lookupUVar (UInternalVar v) = do
  ...
  case envLookup substEnv v of
    ...
    -- this case is needed because, until the rewrite, we have no way to add a
    -- plain name (not an atom) to the substitution, so we do things like `Var
    -- (v:>UnitTy)`.
    Just (Var (v:>_)) -> nameToAtom v
    Just x -> return x
...
inferUDecl (ULet letAnn (UPatAnn p ann) rhs) = do
  ...
  expr <- zonk $ Atom val
  val <- withPatHint p $ emitAnn letAnn expr
  bindPat p val
inferUDecl (UDataDefDecl def tc dcs) = do
  ...
  return $ tc @> nameAsAtom tc' <> newEnv dcs (map nameAsAtom dcs')
inferUDecl (UInterface paramBs superclasses methodTys className methodNames) = do
  ...
  return $  className @> nameAsAtom className'
         <> newEnv methodNames (map nameAsAtom methodNames')
inferUDecl (UInstance argBinders instanceTy methods maybeName) = do
  instanceDict <- checkInstance argBinders instanceTy methods
  case maybeName of
    Nothing -> do
      withNameHint hint $ emitAnn InstanceLet (Atom instanceDict)
      return mempty
    ...
    Just instanceName -> do
      ...
      return $ instanceName @> instanceVal

-- XXX: this is a workaround for the limitation that our substitution carries
-- atoms, not plain names. This is fixed in the safer names rewrite. workaround.
nameAsAtom :: Name -> Atom
nameAsAtom name = Var (name:>UnitTy)
...
inferULam (UPatAnn p ann) arr body = do
  ...
  buildLam (Bind $ patNameHint p :> argTy) arr
    \x@(Var v) -> checkLeaks [v] $ withBindPat p v $ inferSigma body
...
renameForPrinting x = (x, [])
-- renameForPrinting = undefined
