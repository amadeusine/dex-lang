...
module Syntax (
    ...
    IScope, BinderInfo (..), AnyBinderInfo (..),
...
data PassName = Parse | Rename | TypePass | SynthPass | SimpPass | ImpPass | JitPass
...
instance HasUVars UDecl where
  ...
  freeUVars _ = undefined
  -- freeUVars (UDataDefDecl dataDef bTyCon bDataCons) =
  --   freeUVars dataDef <> freeUVars (Abs bTyCon (Abs bDataCons ()))
...
instance BindsUVars UDecl where
  boundUVars decl = case decl of
    ...
    UDataDefDecl _ bTyCon bDataCons -> undefined
      -- boundUVars $ NestPair bTyCon bDataCons
    UInterface _ _ _ _ _     -> mempty
    _ -> undefined
    -- UInstance Nothing  _ _ _ -> mempty
    -- UInstance (Just v) _ _ _ -> uvarSingleton v
...
bindingsAsVars env = [v:>ty | (v, ~(AtomBinderInfo ty _)) <- envPairs env]
...
instance BindsVars Binder where
  ...
  renamingSubst env@(_, scope) b@(Bind (v:>ty)) = (b', env')
    where v' = genFresh v scope
          b' = Bind (v':>ty')
          ty' = subst env ty
          env' = (b@>Var (v':>ty'), b' @> AtomBinderInfo ty' UnknownBinder)
