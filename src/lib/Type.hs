...
instance Subst EvaluatedModule where
  subst env (EvaluatedModule bindings synthCandidates (SourceMap sourceMap)) =
    EvaluatedModule bindings' synthCandidates' $ SourceMap sourceMap'
    where
      ((sourceMap', synthCandidates'), (_, bindings')) = flip runCat env do
         forM_ (envPairs bindings) \(name, info) -> do
           localEnv <- look
           let info' = subst localEnv info
           let name' = genFresh name $ snd localEnv
           -- XXX: we don't have a renaming facility in the dangerous-names
           -- version, so we just use a `UnitTy` atom. It will be ignored anyway.
           extend (name @> Var (name':>UnitTy), name' @> info')
         finalEnv <- look
         let synthCandidates' = subst finalEnv synthCandidates
         sourceMap' <- mapM substOrEmit sourceMap
         return (sourceMap', synthCandidates')

substOrEmit :: Name -> Cat ScopedSubstEnv Name
substOrEmit name = do
  (substEnv, scope) <- look
  case envLookup substEnv name of
    Just atom -> case atom of
